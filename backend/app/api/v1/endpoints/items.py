...
from app.model.items import ItemResponse, ItemBase, ItemCreate, ItemUpdate, Item
...
def read_items(db: SessionDep, skip: int = 0, limit: int = 100,
               ...
               has_categories: Optional[bool] = None, category_id: Optional[int] = None):
    extra_criteria = []
    ...
    return item_service.get_multi(db, skip=skip, limit=limit, name=name,
                                  format=format, criteria=extra_criteria, options=[selectinload(Item.categories)])
